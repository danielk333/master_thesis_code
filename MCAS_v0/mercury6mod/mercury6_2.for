...
c ----------------------------------
c ----------------------------------
c Added some own functionallity such as close6 additional printout
c 
c Copy pasted some functionallity from Joe Hahn's tweek of mercury6
c ----------More speciffically:
c #3. For each body in the system, you can pass up to three optional 
c floating point parameters, by adding "p1=value1 p2=value2 p3=value3"
c to the "name & mass" lines of particles.in (also known as small.in)
c and planets.in (aka big.in). Example:
c JUPITER    m=9.54792E-04 r=3.d0 d=1.33 p1=1.d0 p2=0.2d0 p3=1.0d-5
c These values are stored in the array params(3,nbod), which is passes
c to MFO_USER(), which is the subroutine that calculates accelerations
c due to a user-defined force. That subroutine could then be coded to
c use those parameters of, say, orbit decay due to a drag
c force, planet migration due to external torques, etc.
c
c
c Daniel Kastinen (Feb-2016)
c ----------------------------------
c ----------------------------------
c
c
...
c
      ...
      real*8 params(3,NMAX),global_params(4)
      character*8 id(NMAX)
...
c Get initial conditions and integration parameters
     ...
     %  epoch,ngf,opt,opflag,ngflag,outfile,dumpfile,lmem,mem,params,
     %  global_params)
c
c If this is a new integration, integrate all the objects to a common epoch.
  ...
  20    open (23,file=outfile(3),status='old',access='append',err=20)
     ...
     %    rceh,stat,id,epoch,ngf,opt,ngflag,params,global_params)
...
c Do a final data dump
     ...
     %  id,ngf,epoch,opt,opflag,dumpfile,mem,lmem,params,global_params)
c
...
c------------------------------------------------------------------------------
c
      subroutine mfo_user (time,jcen,nbod,nbig,m,x,v,a,params,
     %  output_flag,global_params)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      integer nbod, nbig, output_flag
      real*8 time,jcen(3),m(nbod),x(3,nbod),v(3,nbod),a(3,nbod)
      real*8 params(3,nbod),global_params(4)
c
c Local
      integer j
      real*8 xt,yt,zt,R2,R
      real*8 vxt,vyt,vzt
      real*8 L0,Q_pr,c0,m_p,rho
      real*8 coef1,coef2,VX_dot
c
c------------------------------------------------------------------------------
c
c     default accelerations are zero
      do j = 1, nbig
        a(1,j) = 0.d0
        a(2,j) = 0.d0
        a(3,j) = 0.d0
...
c Solar LUM
      L0=5.5739511D-12
c PI is already defined
c Speed of light
      c0=173.144632327D0
c Radiation presure efficiency factor
      Q_pr=1.0D0
      do j = nbig+1, nbod
        if (params(1,j).NE.0.AND.params(2,j).NE.0) then
          xt=x(1,j)
          yt=x(2,j)
          zt=x(3,j)
          vxt=v(1,j)
          ...
          rho=params(2,j)*1683.60312587D0
          m_p=params(1,j)/1.98855D30

          R2 = xt*xt + yt*yt + zt*zt
          R = R2**0.5D0
	  
          coef1=((m_p*0.75D0/(PI*rho))**(2.D0/3.D0))
          coef2=L0*Q_pr/(4*R2*c0*m_p)
          VX_dot = (vxt*xt + vyt*yt + vzt*zt)/R

          a(1,j) = coef1*coef2*((1 - VX_dot/c0)*(xt/R) - vxt/c0)
          a(2,j) = coef1*coef2*((1 - VX_dot/c0)*(yt/R) - vyt/c0)
          a(3,j) = coef1*coef2*((1 - VX_dot/c0)*(zt/R) - vzt/c0)
        else
          a(1,j) = 0.d0
          a(2,j) = 0.d0
          a(3,j) = 0.d0
...
c Update the data dump files
     ...
     %    rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,rho,rceh,stat,
     %    id,ngf,epoch,opt,opflag,dumpfile,mem,lmem,params,
     %    global_params)
        tdump = time
      end if
c
...
c Advance one timestep
     ...
     %  rcrit,ngf,stat,dtflag,ngflag,opt,nce,ice,jce,mfo_all,params,
     %  tout,global_params)
      time = time + hdid
c
...
c Remove lost objects, reset flags and recompute Hill and physical radii
     ...
     %    id,mem,lmem,outfile(3),itmp,params)
        dtflag = 1
...
c Check for collisions
     ...
     %  jhit,thit,dhit,algor,ngf,ngflag,params,global_params)
c
...
c
     ...
     %  rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,rho,rceh,
     %  stat,id,ngf,opt,opflag,ngflag,outfile,dumpfile,mem,lmem,onestep,
     %  coord,bcoord,params,global_params)
c
      implicit none
      include 'mercury.inc'
c
...
c Local
      ...
      integer dtflag,ejflag,stopflag,colflag,nstored,output_flag
      ...
      real*8 ixvclo(6,CMAX),jxvclo(6,CMAX),a(NMAX),r
...
c Save the current heliocentric coordinates and velocities
     ...
     %  mem,lmem,params,output_flag,global_params)
      time = time + h0
c
c------------------------------------------------------------------------------
c
...
c Check for collisions with the central body
     ...
     %  nhit,jhit,thit,dhit,algor,ngf,ngflag,params,global_params)
c
c If something hit the central body, restore the coords prior to this step
     ...
     %    ngflag,opt,params,global_params)
        time = time - h0
c
...
c Input/Output
      integer nbod,nbig,ngflag,opt(8)
      real*8 time,jcen(3),h,m(nbod),x(3,nbod),v(3,nbod),xh(3,nbod)
      real*8 vh(3,nbod),ngf(4,nbod),params(3,nbod),global_params(4)
c
c Local
      integer j
c
...
c
     ...
     %  ngflag,opt,params,output_flag,global_params)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      integer nbod,nbig,ngflag,opt(8),output_flag
      real*8 time,jcen(3),h,m(nbod),x(3,nbod),v(3,nbod),xh(3,nbod)
      real*8 vh(3,nbod),ngf(4,nbod),params(3,nbod),global_params(4)
c
c Local
...
c If required, apply non-gravitational and user-defined forces
     ...
     %    ausr,params,output_flag,global_params)
...
c Input/Output
      integer nbod,nbig,ngflag,opt(8),output_flag
      real*8 time,jcen(3),h,m(nbod),xh(3,nbod),vh(3,nbod),x(3,nbod)
      real*8 v(3,nbod),ngf(4,nbod),params(3,nbod),global_params(4)
c
c Local
      integer j,k,iflag,stat(NMAX)
      real*8 minside,msofar,gm(NMAX),a(3,NMAX),xj(3,NMAX),vj(3,NMAX)
...
c If required, apply non-gravitational and user-defined forces
     ...
     %  params,output_flag,global_params)
...
c Calculate accelerations at the start of the step
     ...
     %  ngflag,opt,nce,ice,jce,params,global_params)
c
 100  continue
c
...
c
     ...
     %      ngf,ngflag,opt,nce,ice,jce,params,global_params)
          do k = 2, nbod
...
c
     ...
     %  outfile,mem,lmem,params,output_flag,global_params)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      ...
      integer colflag,lmem(NMESS),nclo,iclo(CMAX),jclo(CMAX),output_flag
      ...
      real*8 ixvclo(6,CMAX),jxvclo(6,CMAX),params(3,nbod)
      real*8 global_params(4)
...
c If objects had close encounters, advance H_K using Bulirsch-Stoer instead
     ...
     %    jxvclo,outfile,mem,lmem,mfo_hkce,params,global_params)
      end if
c
c Advance solar Hamiltonian for H/2
      mvsum(1) = 0.d0
...
c
     ...
     %  ngflag,colflag,ce,nce,ice,jce,nclo,iclo,jclo,dclo,tclo,ixvclo,
     %  jxvclo,outfile,mem,lmem,force,params,global_params)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      ...
      real*8 m(nbod),x(3,nbod),v(3,nbod),s(3,nbod),params(3,nbod)
      real*8 global_params(4)
...
c Save old coordinates and integrate
     ...
     %    ibs,jbs,force,params,global_params)
        tlocal = tlocal + hdid
c
...
c Input/Output
      ...
      real*8 ngf(4,nbod),rphys(nbod),rcrit(nbod),params(3,nbod)
      real*8 global_params(4)
      external force
c
c Local
...
c
     ...
     %  ngflag,opt,nce,ice,jce,params,output_flag,global_params)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      ...
      real*8 a(3,nbod),ngf(4,nbod),rcrit(nbod),params(3,nbod)
      real*8 global_params(4)
c
c Local
      integer j
...
c
     ...
     %  stat,id,ngf,epoch,opt,opflag,dumpfile,mem,lmem,params,
     %  global_params)
c
      implicit none
      include 'mercury.inc'
...
c           Tweak #3: write body parameters p1,p2,p3 to dump file
            do k = 1, 3
              if (params(k,j).ne.0) then
                write (c(len+1:len+16),'(a2,i1,a1,e12.5)') ' p',k,'=',
     %            params(k,j)
                len = len + 16
              end if
            end do
...
c       Lines 172-173 refer to global parameters 1 and 2
c        write (33,'(a)') mem(172)(1:lmem(172))
c        if (opt(7).eq.1) then
c          write (33,'(2a)') mem(173)(1:lmem(173)),mem(6)(1:lmem(6))
c        else
c          write (33,'(2a)') mem(173)(1:lmem(173)),mem(5)(1:lmem(5))
c        end if
        write (33,*) mem(172)(1:lmem(172)),global_params(1)
        write (33,*) mem(173)(1:lmem(173)),global_params(2)
        if (opt(8).eq.1) then
...
c Infrequently-changed parameters
        ...
        write (33,*) mem(182)(1:lmem(182)),global_params(3)
        write (33,*) mem(183)(1:lmem(183)),global_params(4)
...
c       NO relativity
        ...
        if (j.eq.13) read (c80,*,err=661) global_params(1)
        if (j.eq.14) read (c80,*,err=661) global_params(2)
        ...
        if (j.eq.22) read (c80,*,err=661) global_params(3)
        if (j.eq.23) read (c80,*,err=661) global_params(4)
...
c
c Read values of mass, close-encounter limit, density, 
c optional parameters p1,p2,p3, etc.
        do k = 3, nsub, 2
     ...
     %        .eq.'Ep') then
            ...
            ngf (4,nbod) = temp
c         Tweak #3: read body's optional parameters p1,p2,p3
          else if (c80(1:2).eq.'p3'.or.c80(1:2).eq.'P3') then
            params(3,nbod) = temp
          else if (c80(1:2).eq.'p1'.or.c80(1:2).eq.'P1') then
            params(1,nbod) = temp
          else if (c80(1:2).eq.'p2'.or.c80(1:2).eq.'P2') then
            params(2,nbod) = temp
          else
            goto 666
          end if
...
c       Tweak #5: write 4 global parameters
        write (23,'(a,1p1e11.4)') mem(172)(1:lmem(172)),global_params(1)
        write (23,'(a,1p1e11.4)') mem(173)(1:lmem(173)),global_params(2)
        write (23,'(a,1p1e11.4)') mem(182)(1:lmem(182)),global_params(3)
        write (23,'(a,1p1e11.4)') mem(183)(1:lmem(183)),global_params(4)
c
...
c
     ...
     %  id,mem,lmem,outfile,nelim,params)
c
      implicit none
      include 'mercury.inc'
c
...
c
     ...
     %  rho,rceh,stat,id,epoch,ngf,opt,ngflag,params,global_params)
c
      implicit none
      include 'mercury.inc'
c
