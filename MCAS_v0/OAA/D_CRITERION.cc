...
double d_SH(const OBSERVATION_record *orb1, const OBSERVATION_record *orb2) {
   ...
   if(!is_finite(d)) {
      if((abs(e_a - e_b) + abs(q_a - q_b) + abs(i_a - i_b) + abs(omega_a - omega_b) + abs(Omega_a - Omega_b)) < 1e-7 ) {
         d = 0;
      }
      else {
         std::cout << std::endl << std::endl;
         std::cout << "WARNING VALUE NOT FINITE AT:" << std::endl << "   double d_SH(const OBSERVATION_record *orb1, const OBSERVATION_record *orb2)" << std::endl;
         std::cout << "d = " << d << std::endl;
         OBSERVATION_record tem_O;
         tem_O = (*orb1);
         tem_O.print();
         tem_O = (*orb2);
         tem_O.print();
         exit(VALUE_NOT_FINITE);
      }
   }
   return d;
}
...
double d_D(const OBSERVATION_record *orb1, const OBSERVATION_record *orb2) {
   ...
   if(!is_finite(d)) {
      if((abs(e_a - e_b) + abs(q_a - q_b) + abs(i_a - i_b) + abs(omega_a - omega_b) + abs(Omega_a - Omega_b)) < 1e-7 ) {
         ...
         std::cout << std::endl << std::endl;
         std::cout << "WARNING VALUE NOT FINITE AT:" << std::endl << "   double d_D(const OBSERVATION_record *orb1, const OBSERVATION_record *orb2)" << std::endl;
         std::cout << "d = " << d << std::endl;
         OBSERVATION_record tem_O;
         tem_O = (*orb1);
         tem_O.print();
...
double rho2(const OBSERVATION_record *orb1, const OBSERVATION_record *orb2) {
   ...
   if(!is_finite(d)) {
      if((abs(e_a - e_b) + abs(q_a - q_b) + abs(i_a - i_b) + abs(omega_a - omega_b) + abs(Omega_a - Omega_b)) < 1e-7 ) {
         ...
         std::cout << std::endl << std::endl;
         std::cout << "WARNING VALUE NOT FINITE AT:" << std::endl << "   double rho2(const OBSERVATION_record *orb1, const OBSERVATION_record *orb2)" << std::endl;
         std::cout << "d = " << d << std::endl;
         OBSERVATION_record tem_O;
         tem_O = (*orb1);
         tem_O.print();
...
double d_N(const OBSERVATION_record *orb1, const OBSERVATION_record *orb2) {
   double v_g_a = (*orb1).v_g;
   double v_g_b = (*orb2).v_g;
   double ra_a = (*orb1).ra;
   double ra_b = (*orb2).ra;
   double dec_a = (*orb1).dec;
   double dec_b = (*orb2).dec;
   double lambda_a = (*orb1).lambda;
   double lambda_b = (*orb2).lambda;

   double w_1 = 1,w_2 = 1,w_3 = 1;

   double Ua,Ub;
   double theta_a,theta_b;
   double phi_a,phi_b;
   std::vector<double> U_a,U_b;
   U_a.push_back(v_g_a/29.7*(-cos(ra_a)*cos(dec_a)));
   U_a.push_back(v_g_a/29.7*(-cos(ra_a)*sin(dec_a)));
   U_a.push_back(v_g_a/29.7*(sin(ra_a)));

   U_a = rot_z(rot_y(U_a,23.4*PI/180),lambda_a);
   Ua = abs_v(U_a);
   theta_a = acos(U_a[1]/Ua);
   phi_a = acos(U_a[0]/Ua);

   U_b.push_back(v_g_b/29.7*(-cos(ra_b)*cos(dec_b)));
   U_b.push_back(v_g_b/29.7*(-cos(ra_b)*sin(dec_b)));
   U_b.push_back(v_g_b/29.7*(sin(ra_b)));

   U_b = rot_z(rot_y(U_b,23.4*PI/180),lambda_b);
   Ub = abs_v(U_b);
   theta_b = acos(U_b[1]/Ub);
   phi_b = acos(U_b[0]/Ub);

   double Delta_phiI,Delta_phiII;
   double Delta_lambdaI,Delta_lambdaII;

   Delta_phiI = 2.0*sin((phi_b-phi_a)*0.5);
   Delta_phiII = 2.0*sin((PI + phi_b - phi_a)*0.5);

   Delta_lambdaI = 2.0*sin((lambda_b-lambda_a)*0.5);
   Delta_lambdaII = 2.0*sin((PI + lambda_b - lambda_a)*0.5);

   std::vector<double> xi_opt;
   xi_opt.push_back(w_2*Delta_phiI*Delta_phiI + w_3*Delta_lambdaI*Delta_lambdaI);
   xi_opt.push_back(w_2*Delta_phiII*Delta_phiII + w_3*Delta_lambdaII*Delta_lambdaII);
   double Delta_XI2 = min_v(xi_opt);

   double d = pow(Ub-Ua,2) + w_1*pow(cos(theta_b) - cos(theta_a),2) + Delta_XI2;
   if(!is_finite(d)) {
      if((abs(v_g_a - v_g_b) + abs(ra_a - ra_b) + abs(dec_a - dec_b) + abs(lambda_a - lambda_b)) < 1e-7 ) {
         d = 0;
      }
      else {
         std::cout << std::endl << std::endl;
         std::cout << "WARNING VALUE NOT FINITE AT:" << std::endl << "   double d_N(const OBSERVATION_record *orb1, const OBSERVATION_record *orb2)" << std::endl;
         std::cout << "d = " << d << std::endl;
         OBSERVATION_record tem_O;
         tem_O = (*orb1);
         tem_O.print();
...
double varrho1(const OBSERVATION_record *orb1, const OBSERVATION_record *orb2) {
   ...
   if(!is_finite(d)) {
      if((abs(e_a - e_b) + abs(q_a - q_b) + abs(i_a - i_b) + abs(omega_a - omega_b) + abs(Omega_a - Omega_b)) < 1e-7 ) {
         ...
         std::cout << std::endl << std::endl;
         std::cout << "WARNING VALUE NOT FINITE AT:" << std::endl << "   double varrho1(const OBSERVATION_record *orb1, const OBSERVATION_record *orb2)" << std::endl;
         std::cout << "d = " << d << std::endl;
         OBSERVATION_record tem_O;
         tem_O = (*orb1);
         tem_O.print();
