	...
	for(i=0; i < OBJS-1; i++) {
		if((*type_vec)[i] == 1) {
			...
			for(j=i+1; j < OBJS; j++) {
				...
				temp_kick_x[i].push_back( Ggrav*((*m_vector)[i])*((*m_vector)[j])*( (*Z)[ind][i*3  ] - (*Z)[ind][j*3  ] )/temp_dist);
				temp_kick_y[i].push_back( Ggrav*((*m_vector)[i])*((*m_vector)[j])*( (*Z)[ind][i*3+1] - (*Z)[ind][j*3+1] )/temp_dist);
				temp_kick_z[i].push_back( Ggrav*((*m_vector)[i])*((*m_vector)[j])*( (*Z)[ind][i*3+2] - (*Z)[ind][j*3+2] )/temp_dist);
			}
